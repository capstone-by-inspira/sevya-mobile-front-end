...
const Notes = () => {
  ...
  const addNote = async () => {
    ...
    // Scroll to the bottom after adding a note
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.innerContainer}>
          ...
          >
              ...
              notes.map((item, index) => (
                <View key={index} style={styles.noteBubble}>
                  <View style={styles.noteHeader}>
                    ...
                    />
                    <Text style={styles.noteAuthor}>
                      By {item.caregiverFirstName} {item.caregiverLastName}
                    </Text>
                  </View>
            ...
            {/* Show loading indicator at the bottom while keeping messages
            visible */}
            {loading && (
...
const styles = StyleSheet.create({
  ...
  loadingCon: {
    ...
    width: 50,
    position: "absolute",
    bottom: 10, // Keeps it at the bottom of the screen
    left: "80%",
  ...
  loadingText: {
    ...
    fontSize: 12,
    fontWeight: "bold",
    marginLeft: 10, // Instead of gap, use margin
  },
});
